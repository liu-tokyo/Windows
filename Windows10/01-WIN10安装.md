...
- 安装最新版本
  ...
  ※ 2024年12月份，已经是 `v1.2.10691` → `v1.9.25200`。为啥我的 Windows10 已经自动更新为最新版本，而 Windows11 却不会自动升级为最新版本呢？
...
- 转移休眠文件位置：  
  找到如下注册表的位置：`HKEY_LOCAL_MACHINE\SYSTEM\ControlSet001\Control\BackupRestore\FilesNotToBackup\Power Management`  
  把默认数值的 `\hiberfil.sys` 修改为 `D:\hiberfil.sys` （需要先禁用休眠，重启后再恢复休眠hiberfil.sys文件就改到了别的路径）。
