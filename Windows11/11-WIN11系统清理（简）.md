...
4. 前述的驱动清理，在几乎所有的电脑执行都没有问题，但是 `Thinkpad` 笔记本存在问题；原因是 ThinkPad E480 的内置键盘、触摸板以及 TrackPoint（红点）在底层并非全走传统的 PS/2 或标准 USB 总线，而是通过 **Intel Serial IO (I2C/GPIO) 控制器** 与 **Lenovo ACPI 驱动** 进行通信。而其他部分笔记本的键盘走的是微软内置的标准 ACPI PS/2 兼容驱动（`keyboard.inf` / `msmouse.inf`），因此不会受到影响。
