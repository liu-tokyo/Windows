...
```powershell
Get-AppxProvisionedPackage -Online | Where-Object DisplayName -like "Microsoft.ZuneMusic" | Remove-AppxProvisionedPackage -Online -ErrorAction SilentlyContinue
```
...
foreach ($p in $patterns) {
    Get-AppxProvisionedPackage -Online | Where-Object DisplayName -like $p | Remove-AppxProvisionedPackage -Online -ErrorAction SilentlyContinue
}

```

```powershell
"HEIFImageExtension","ScreenSketch","VP9VideoExtensions","WebMediaExtension","WebpImageExtension","Windows.DevHome","Windows.Photos","WindowsCalculator","WindowsCamera","YourPhone","MPEG2VideoExtension","RawImageExtension","AVCEncoderVideoExtension","AV1VideoExtension","HEVCVideoExtension","Windows.Client.WebExperience" | ForEach-Object {
    Get-AppxProvisionedPackage -Online | Where-Object DisplayName -like "*$_*" | Remove-AppxProvisionedPackage -Online -ErrorAction SilentlyContinue
}
```

**彻底从系统中清除（即使系统封装之后也不会再被安装）- 21H2**
...
- 使用如下指令，把所有组件更新到最新状态：
  ...
  如果因为 imdisk 的内存盘作为临时缓存，出现错误话，重新指定临时缓存路径：
