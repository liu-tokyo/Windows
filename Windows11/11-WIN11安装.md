...
**方法1**：新版本的WIN11已经无法在安装的时候使用本地用户，需要如下操作才可以。
...
- **`uBlock Origin`**
  ...
  推荐理由：公认最轻量、最强大的广告拦截插件。相比其他同类工具占用内存更少，能彻底屏蔽各类网页弹窗、视频广告和追踪脚本。
...
- 浏览器缓存转移（EDGE）
  ...
  - 删除 `C:\Users\%username%\AppData\Local\Microsoft\Edge\User Data\Default\` 目录的 `Cache` 子目录

    ```powershell
    rmdir /S /Q "C:\Users\%username%\AppData\Local\Microsoft\Edge\User Data\Default\Cache"
    ```
...
- 浏览器缓存转移（CHROME）
  ...
  - 删除 `C:\Users\%username%\AppData\Local\Google\Chrome\User Data\Default\` 目录的 `Cache` 子目录
